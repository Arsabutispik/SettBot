...
export default async(guild: Guild, user: User, action: "UYARI" | "BAN" | "AT" | "SUSTUR" | "ZORUNLU_BAN" | "SÜRELİ_BAN" | "DEĞİŞİKLİK" | "BAN_KALDIR" | "BAN_SÜRESİ", actionmaker: User, reason: string, duration?: number, casenumber?: number) => {
    ...
    if(action == "UYARI") {
        ...
        const amount = ms(duration!, {long: true}).replaceAll(/seconds|second/, "saniye").replaceAll(/minutes|minute/, "dakika").replaceAll(/hours|hour/, "saat").replaceAll(/day|days/, "gün")
